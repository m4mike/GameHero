        ...
        api.tictacMp = {
            gamePrototype: function (data) {
                if (data == null) {
                    ...
                    this.turn = 0;
                    this.state = "playing";
                    this.attackerMarker = "X";
                    this.defendantMarker = "O";
            ...
            saveGame: function ( game, callback) {
                ...
                var gamedata = {
                    ...
                    turn: game.turn,
                    state: game.state,
                    attackerMarker: game.attackerMarker,
                    defendantMarker: game.defendantMarker,
                    attackerId: game.attackerId,
                    defendantId:game.defendantId,
                    id:key
                }
            ...
            determineGameState: function (game) {
                ...
                for (var i in game.board) {
                    ...
                    if (row[0] != null && row[0] == row[1] && row[0] == row[2]) {
                        return api.tictacMp.determineWinningState(game, row[0]);
                    }
                }                ;
                // vertical win
                var i = 0;
                while (i <= 2) {
                    if (game.board[0][i] != null && game.board[0][i] == game.board[1][i] && game.board[0][i] == game.board[2][i]) {
                        return api.tictacMp.determineWinningState(game, game.board[0][i]);
                    }
                    i++;
                }
                // diagonal win
          ...
          || (game.board[1][1] != null && game.board[0][2] == game.board[1][1] && game.board[0][2] == game.board[2][0])
                    ...
                    return api.tictacMp.determineWinningState(game, game.board[1][1]);
                }
                // tie
                var tied = true;
                ...
                game.board.forEach(function (row) {
                    ...
                    row.forEach(function (square) {
                        ...
                        if (square != null) {
                            ...
                            if (api.tictac.determineGameState(proposedGameAi) == "attacker lost") {
                                options[y][x] = 1 // I win
                                if (1 > bestScore || bestScore == null) {
                                    bestScore = 1;
                                    bestMove = [y, x]
                                }
                            } else if (api.tictac.determineGameState(proposedGamePlayer) == "attacker won") {

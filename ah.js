...
//modified for custom start
fs.readdirSync(binary.paths.projectRoot + '/methods/').sort().forEach(function(file){
  if(file.indexOf('.js') > 0){
    var action = file.split('.')[0];
    binary.actions[action] = require(binary.paths.projectRoot + '/methods/' + file)[action];
  }
});

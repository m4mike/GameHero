...
exports.startCluster = function (binary) {
    ...
    async.series({
        ...
        setup: function (next) {
            ...
            if (binary.numWorkers < 2) { binary.numWorkers = 1 }
        ...
        displaySetup: function (next) {
            ...
            binary.log('  Num cpus = ' + binary.numCPUs, 'info');
            binary.log('  Workers = ' + binary.numWorkers, 'info');
            
            binary.log('pid: ' + process.pid, 'notice');
            binary.log('options:', 'debug');
            for (var i in binary.clusterConfig) {
                binary.log(' > ' + i + ': ' + binary.clusterConfig[i], 'info');
            }
            next();
        },
